...
class BlackListBackend(object):
    ...
    def get_ip_lockup_time_remaining(self, ip_user):
        return (
        (ip_user.datum_zaklepanja + timedelta(hours=settings.LOGIN_LOCKUP_TIME)) - datetime.now()).total_seconds()
    ...
    def login_allowed(self, ip):
        ip_user = self.add_get_ip_user(ip=ip)
        return (self.get_ip_lockup_time(
            ip_user) / 3600) >= settings.LOGIN_LOCKUP_TIME or ip_user.poiskusi <= settings.MAX_LOGIN_ATTEMPTS
